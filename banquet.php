...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (!empty($_POST['date']) && !empty($_POST['persons']) && !empty($_POST['eventType']) && !empty($_POST['days']) && !empty($_POST['totalRent'])) {
        ...
        if ($stmt->execute()) {
            echo "<script>alert('Booking confirmed!'); window.location.href='payment.html';</script>";
        } else {
...
<head>
    ...
    <style>
        ...
        .container { max-width: 800px; margin: 50px auto; background: white; padding: 20px; border-radius: 10px; box-shadow: 0px 0px 10px rgba(0, 0, 0, 0.1); text-align: center; }
        h1, h2 { color: #333; }
        p { color: #666; }
        .form-group { margin: 15px 0; text-align: left; }
        label { display: block; margin-bottom: 5px; font-weight: bold; }
        input { width: calc(100% - 16px); padding: 10px; border: 1px solid #ccc; border-radius: 5px; }
        .rent { font-size: 20px; font-weight: bold; color: green; }
        ...
        .image-gallery { display: flex; justify-content: center; gap: 10px; margin-top: 20px; }
        .image-gallery img { width: 30%; border-radius: 10px; }
    </style>
</head>
<body>
    <div class="container">
        <h1>Banquet Hall Booking</h1>
        <p>Welcome to our luxurious banquet hall. Perfect for weddings, parties, and corporate events.</p>
        <p>Capacity: Up to 1000 people</p>

        <div class="image-gallery">
            <img src="logo.jpg" alt="Banquet Hall 1">
            <img src="logo.jpg" alt="Banquet Hall 2">
            <img src="logo.jpg" alt="Banquet Hall 3">
        </div>
