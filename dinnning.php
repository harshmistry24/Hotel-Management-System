...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Prepare SQL query
    $stmt = $conn->prepare("INSERT INTO bookings (first_name, last_name, contact, guests, date, time) VALUES (?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("ssssss", $first_name, $last_name, $full_contact, $guests, $date, $time);

    if ($stmt->execute()) {
        echo "<script>alert('Booking confirmed!'); window.location.href='dinnning.php';</script>";
    } else {
...
<body>
    <div class="booking-container">
        ...
        <form method="post" action="./dinnning.php">
